...
def register():
    try:
        ...
        if request.method == 'POST':    
            ...
            content = ' to activate your account, please open link ' + flask.url_for('auth.activate', _external=True) + '?auth=' + number
            
            print(content)

            #send_email(credentials, receiver=email, subject='Activate your account', message=content)
            
            flash('Hello there' + content)
...
def change():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT userid FROM forgotlink WHERE challenge=? AND state=?', (number, utils.F_ACTIVE)
            ).fetchone()
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if user is not None:
                ...
                content = ' to change your password, please open this link ' + flask.url_for('auth.change', _external=True) + '?auth=' + number
